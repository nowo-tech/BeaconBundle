...
- **Do not commit real DSNs**. A DSN contains a public key and may optionally contain a secret (`https://PUBLIC[:SECRET]@host/project`). Treat the full DSN as sensitive configuration and keep it in env vars or your secret manager.
...
| Optional DSN secret | Secret embedded in DSN copied into public docs or shell history | Prefer secret storage, avoid pasting secrets in shared terminals or examples |
...
| **Logging** | No code path logs full DSNs, optional secrets, or unrelated request secrets. |
